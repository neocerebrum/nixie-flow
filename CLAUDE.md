...
The app is fully internationalized. Supported languages: EN (fallback), IT, FR, DE, ES, PT, ZH, JA, KO.

- **Translation files**: `lang/{en,it,fr,de,es,pt,zh,ja,ko}.php` — flat associative arrays with dot-notation keys.
...
- **I18n class**: `app/I18n.php` — detection chain: cookie `nixieflow_lang` → `Accept-Language` header → fallback `en`.
